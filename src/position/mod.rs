use std::{fmt::Write, ops::{BitAndAssign, BitOrAssign, BitOr, Shl, ShlAssign, BitAnd}, fs::Metadata};
...
impl Square {
  ...
  pub fn as_board_string(&self) -> String {
    ...
    for rank in 0u8..8 {
      let _ = write!(&mut board, "{} ", 8 - rank);
      let rank = 8 * (8 - rank - 1);
      for file in 0u8..8 {
        let pos = (1u64 << file) << rank;
        if pos == self.0 {
          let _ = write!(&mut board, "X ");
        } else {
          let _ = write! (&mut board, "- ");
        }
      }
      let _ = write!(&mut board, "\n");
    }
    let _ = write!(&mut board, "  A B C D E F G H");
  ...
  pub fn from_fen(fen: &str) -> Self {
    ...
    for position in fen.chars() {
      ...
      if position.is_numeric() {
        file += (position as u8) - ('0' as u8);
      }

      if position.is_ascii_lowercase() {
...
impl std::fmt::Debug for PositionMetadata {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
      ...
      f.debug_struct("PositionMetadata")
        ...
        .field("En Passant Square", &format!("{}{en_passant_rank}", en_passant_file as char))
        .finish()
    }
}
  ...
  pub fn print_board(&self) {
    ...
    for rank in 0u8..8 {
      for file in 0u8..8 {
        ...
        let piece_offset = 
          if self.get_board_for_piece(Piece::King) & sweep > 0 {
            0
