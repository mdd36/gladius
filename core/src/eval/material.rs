...
pub fn material_score(position: &Position, color: Color) -> i16 {
	...
	let num_knights = (position.get_board_for_piece(Piece::Knight) & color_board).count_ones() as i16;
	let num_bishops = (position.get_board_for_piece(Piece::Bishop) & color_board).count_ones() as i16;
	...
	let bishop_pair_bonus = if num_bishops > 1 {
		HALF_PAWN
	} else {
		0
	};
