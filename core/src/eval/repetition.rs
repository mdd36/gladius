...
pub fn is_threefold_repetition(ply_from_irreversible: u8, ply_from_root: u8, move_history: &Vec<u64>, new_position: u64) -> bool {
	if ply_from_irreversible > 4 {
		let start = move_history.len() - ply_from_irreversible as usize;
		let repetition_count = move_history[start..].iter()
