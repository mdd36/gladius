...
fn parse_position(input: &str, mut cmd: SplitAsciiWhitespace<'_>) -> Result<UciCommand, &'static str> {
  ...
  while let Some(word) = cmd.next() {
    match word {
      "fen" => {
        ...
        let parsed_position = Position::from_fen(&input[start..end])
          .map_err(|_| "Failed to parse fen")?;
